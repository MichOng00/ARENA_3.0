...
class SGD:
    ...
    def step(self) -> None:
        ...
        #     g = self.params[t].grad
            
        #     if self.lmda != 0:
        #         g = g + self.lmda*self.params[t]
        #     if self.mu != 0:
        #         self.b[t] = self.mu*self.b[t] + g
        #         g = self.b[t]
        #     self.params[t] = self.params[t] - self.lr * g
...
class Adam:
    ...
    def step(self) -> None:
        for m, theta, v in zip(self.m, self.params, self.v):
            ...
            theta -= self.lr * m / (v**0.5 + self.eps)  # inplace operation, to modify params
        self.t += 1
...
class AdamW:
    ...
    def step(self) -> None:
        for m, theta, v in zip(self.m, self.params, self.v):
            g = theta.grad
            theta *= 1 - self.lmda*self.lr
            m.copy_(self.beta1 * m + (1-self.beta1) * g)
            v.copy_(self.beta2 * v + (1-self.beta2) * g**2)
            m_hat = m/(1 - self.beta1 ** self.t)
            v_hat = v/(1 - self.beta2 ** self.t)
            
            theta -= self.lr * m_hat / (v_hat**0.5 + self.eps)  # inplace operation, to modify params
        self.t += 1
...
class ResNetFinetuner:
    ...
    def pre_training_setup(self):
        """Loads the model, optimizer and data."""
        # not in __init__ for flexibility later
...
# random (vs grid) gives more distinct values in the important hyperparams
sweep_config = dict(
    method = "random",
