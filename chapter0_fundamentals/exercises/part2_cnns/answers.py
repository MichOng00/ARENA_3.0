...
#%%
# a prototypical nn.Module subclass
class MyModule(nn.Module):
    def __init__(self, weights: Tensor, biases: Tensor):
        super().__init__()
        # wrapping weights and biases in nn.Parameter
        # so torch detects and updates them automatically
    ...
    def extra_repr(self) -> str:
        # print out useful info - for more complex modules
...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        """
        super().__init__()
        n_in_root = in_features**0.5
        self.weight = nn.Parameter(t.rand(out_features, in_features) * 2/n_in_root + 1/n_in_root)
        if bias:
            self.bias = nn.Parameter(t.rand(out_features) * 2/n_in_root + 1/n_in_root)
        else:
            self.bias = None
...
class SimpleMLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return self.linear2(self.relu(self.linear1(self.flat(x))))
...
# total can be necessary if iterable does not have length
t0 = time.time()
...
class Conv2d(nn.Module):
    def __init__(
        ...
        denom = (self.in_channels)**0.5 * self.kernel_size
        self.weight = nn.Parameter(
            (t.rand(self.out_channels, self.in_channels, self.kernel_size, self.kernel_size)*2 - 1)/denom
        )
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # return ((x - mean) / var)*self.weight + self.bias
        ## from solution
...
class BlockGroup(nn.Module):
    def __init__(self, n_blocks: int, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.subsequent = Sequential(*[ResidualBlock(out_feats, out_feats) for _ in range(n_blocks-1)])
...
class ResNet34(nn.Module):
    def __init__(
        ...
        self.blocks = Sequential(
            *[BlockGroup(self.n_blocks_per_group[i], 
                         [out_feats0, *self.out_features_per_group][i],
                         self.out_features_per_group[i],
                         self.first_strides_per_group[i])
                for i in range(len(self.n_blocks_per_group))]
        )
        self.remaining = Sequential(
            AveragePool(),
